...
namespace ValheimPlus.AntiAnyMods
{
    internal class Rpc
    {
        ...
        public static void AcHandshake(ZRpc rpc, long sender, string hash)
        {
            ...
            {
                Rpc.Clients.Add(sender, ValheimPlusPlugin.PluginsHash.Equals(hash));
        ...
        {
            ZLog.Log(ZNet.instance.IsServer() ? (object) "Server" : (object) ("Clientreceived AcRoutedHandshake from " + sender.ToString()));
            if (ZNet.instance.IsServer())
                ZLog.Log((object) ("AC: Storing " + sender.ToString()));
            else
                ZLog.Log((object) "AC: Got server request, sending hash");
