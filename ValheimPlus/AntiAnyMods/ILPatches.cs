...
namespace ValheimPlus.AntiAnyMods
{
  internal class ILPatches
  {
      ...
      IEnumerable<CodeInstruction> instructions)
    {
      ...
      {
        ZLog.LogError((object) "[AntiCheat] Could not find ZPackage:ZPackage, ZPackage:Write(string) or ZPackage:Write(long)!");
        return instructions;
      }
      ...
      if (lastIndex == -1)
      {
        ZLog.LogError((object) "[AntiCheat] Could not get ZPackage:ZPackage() instruction!");
      ...
      codeInstructionList.InsertRange(lastIndex + 1, (IEnumerable<CodeInstruction>) new CodeInstruction[3]
      {
        ...
        new CodeInstruction(OpCodes.Ldstr, (object) ValheimPlusPlugin.PluginsHash),
      ...
      IEnumerable<CodeInstruction> instructions)
    {
      ...
      if (lastIndex == -1)
      {
        ZLog.LogError((object) "[AntiCheat] Could not get FejdStartup:ShowConnectError() instruction!");
        return (IEnumerable<CodeInstruction>) codeInstructionList;
      }
      codeInstructionList.InsertRange(lastIndex, (IEnumerable<CodeInstruction>) new CodeInstruction[5]
      {
        new CodeInstruction(OpCodes.Ldarg_0),
        new CodeInstruction(OpCodes.Ldfld, (object) ValheimPlusPlugin.PluginsHash),
        new CodeInstruction(OpCodes.Ldstr, (object) "[AntiCheat]: You have been kicked for using disallowed mods"),
