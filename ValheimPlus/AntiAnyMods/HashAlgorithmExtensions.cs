...
namespace ValheimPlus.AntiAnyMods
{
    public static class HashAlgorithmExtensions
    {
        ...
        {
            List<string> list = ((IEnumerable<string>) Directory.GetFiles(path, "*.dll", SearchOption.AllDirectories)).OrderBy<string, string>((Func<string, string>) (p => p)).ToList<string>();
            MD5 md5 = MD5.Create();
